 ...
 */
export function formatHoursMinutes(time: number): HoursMinutes {
 ...
 */
export function generateTimeDisplayText(time: number, coldProof: number): string {
 ...
 */
export function generateTimeRangeInfo(
    minTime: number,
    maxTime: number,
    coldProof: number,
    fridgeTemp: number,
 ...
 */
export function generateRecipeCardsHTML(
    _foldingSchedule: { times: number[]; count: number },
    _bakingTimes: { preheatTime: number; steamTime: number; totalBakeTime: number },
    _hydration: number,
    _numLoaves: number
): string {
 ...
 */
export function generateInfoText(temp: number, starterPercent: number): string {
    let info = '';
 ...
 */
export function announceToScreenReader(message: string, duration: number = 3000): void {
 ...
 */
export function updateBreadEmojis(): void {
    const numLoaves = parseFloat((document.getElementById('numLoaves') as HTMLInputElement | null)?.value || '1') || 1;
 ...
 */
export function updateWholeGrainPercent(): void {
    const flour = parseFloat((document.getElementById('flour') as HTMLInputElement | null)?.value || '0') || 0;
 ...
 */
export function updateRecipeSummary(): void {
    const flour = parseFloat((document.getElementById('flour') as HTMLInputElement | null)?.value || '0') || 0;
    const water = parseFloat((document.getElementById('water') as HTMLInputElement | null)?.value || '0') || 0;
    const starter = parseFloat((document.getElementById('starter') as HTMLInputElement | null)?.value || '0') || 0;
    const salt = parseFloat((document.getElementById('salt') as HTMLInputElement | null)?.value || '0') || 0;
    ...
    if (flourPercentEl) flourPercentEl.textContent = '100%';
    if (waterPercentEl) waterPercentEl.textContent = `${hydrationPercent}% hydrering`;
    if (starterPercentEl) starterPercentEl.textContent = `${starterPercent}%`;
    if (saltPercentEl) saltPercentEl.textContent = `${saltPercent}%`;
