/**
 * Baking schedule generation and management
 * Complete TypeScript migration from schedule.js
 */
...
/**
 * Type definitions for schedule functionality
 */

type MessageType = 'warning' | 'error' | 'success' | 'info';

type TimeDirection = 'forward' | 'backward';

type DataSource = 'calculated' | 'manual';

type BakingMethod = 'dutch-oven' | 'steam';

type BulkPhase = 'start' | 'active' | 'end';
...
declare global {
    interface Window {
        ...
        toggleScheduleInputs: () => void;
        generateBakingSchedule: (autoGenerate?: boolean) => void;
        toggleStepCheck: (stepId: string) => void;
        startBulkTimerFromSchedule: (durationHours: number) => void;
        removeBulkTimer: () => void;
        updateTimerButtons: () => void;
        updateFoldsRecommendation: () => void;
        ...
        resumeTimer?: () => void;
        restartTimer?: () => void;
        showActionToast?: (config: { emoji: string; text: string }, duration: number) => void;
    }
}
...
function showInlineMessage(
    ...
    containerId: string = 'scheduleOutput',
    duration: number = 0
): void {
 ...
 */
function formatTimeWithDate(date: Date, referenceDate: Date): string {
 ...
 */
function getDayNumber(date: Date, startDate: Date): number {
 ...
 */
window.toggleTimeDirection = function(): void {
    const direction = (document.querySelector('input[name="timeDirection"]:checked') as HTMLInputElement | null)?.value as TimeDirection | undefined;
 ...
 */
window.goToScheduleTab = function(): void {
 ...
 */
function tryAutoGenerateSchedule(): void {
    ...
    const dataSource = (document.querySelector('input[name="scheduleDataSource"]:checked') as HTMLInputElement | null)?.value as DataSource | undefined;
    ...
    if (dataSource === 'calculated' && !SourdoughApp.calculatedTime) {
        const flour = parseFloat((document.getElementById('flour') as HTMLInputElement | null)?.value || '0');
        const water = parseFloat((document.getElementById('water') as HTMLInputElement | null)?.value || '0');
    ...
    window.generateBakingSchedule(true); // Pass true to indicate auto-generation
}

/**
 * Setup listener for when schedule tab becomes active
 */
function setupTabSwitchListener(): void {
 ...
 */
function setupScheduleLiveUpdates(): void {
    // Time input
    const timeInput = document.getElementById('scheduleTime');
    if (timeInput) {
        timeInput.addEventListener('change', () => {
            window.toggleTimeDirection(); // Update hint with new time
            tryAutoGenerateSchedule();
        });
    ...
    manualInputs.forEach(id => {
        ...
        if (input) {
            input.addEventListener('change', () => {
                const dataSource = (document.querySelector('input[name="scheduleDataSource"]:checked') as HTMLInputElement | null)?.value;
 ...
 */
function updateFoldsRecommendation(): void {
    const flour = parseFloat((document.getElementById('flour') as HTMLInputElement | null)?.value || '500');
    const water = parseFloat((document.getElementById('water') as HTMLInputElement | null)?.value || '350');
    ...
    const foldsSelect = document.getElementById('scheduleFolds') as HTMLSelectElement | null;
 ...
 */
function updateAutolysFromCalculator(): void {
    const autolyseValue = (document.getElementById('autolyse') as HTMLSelectElement | null)?.value;
    const scheduleAutolysCheckbox = document.getElementById('scheduleAutolys') as HTMLInputElement | null;
    const autolysTimeSelect = document.getElementById('autolysTime') as HTMLSelectElement | null;
 ...
 */
window.toggleScheduleInputs = function(): void {
    const dataSource = (document.querySelector('input[name="scheduleDataSource"]:checked') as HTMLInputElement | null)?.value;
 ...
 */
window.generateBakingSchedule = function(autoGenerate: boolean = false): void {
    // Show loading state
    showScheduleLoading();
...
function generateScheduleInternal(autoGenerate: boolean = false): void {
    ...
    const timeDirection = ((document.querySelector('input[name="timeDirection"]:checked') as HTMLInputElement | null)?.value || 'forward') as TimeDirection;

    // Get values based on data source
    let bulkTime: number, coldProof: number, numLoaves: number, weightPerLoaf: number, flour: number, water: number, hydration: number;
    ...
    if (dataSource === 'calculated') {
        ...
        coldProof = parseFloat((document.getElementById('coldProof') as HTMLInputElement | null)?.value || '0');
        flour = parseFloat((document.getElementById('flour') as HTMLInputElement | null)?.value || '500');
        water = parseFloat((document.getElementById('water') as HTMLInputElement | null)?.value || '350');
        hydration = (water / flour) * 100;
        numLoaves = parseFloat((document.getElementById('scheduleNumLoavesMain') as HTMLInputElement | null)?.value || '2');

        const starter = parseFloat((document.getElementById('starter') as HTMLInputElement | null)?.value || '100');
        const salt = parseFloat((document.getElementById('salt') as HTMLInputElement | null)?.value || '10');
        const totalWeight = flour + water + starter + salt;
        weightPerLoaf = totalWeight / numLoaves;

    } else {
        // Get manual values
        bulkTime = parseFloat((document.getElementById('manualBulkTime') as HTMLInputElement | null)?.value || '5');
        coldProof = (document.getElementById('manualColdProof') as HTMLInputElement | null)?.checked
            ? parseFloat((document.getElementById('manualColdProofTime') as HTMLInputElement | null)?.value || '0')
            : 0;
        numLoaves = parseFloat((document.getElementById('manualNumLoaves') as HTMLInputElement | null)?.value || '1');
        weightPerLoaf = parseFloat((document.getElementById('manualLoafWeight') as HTMLInputElement | null)?.value || '800');
    ...
    const useAutolys = (document.getElementById('scheduleAutolys') as HTMLInputElement | null)?.checked || false;
    const autolysTime = parseInt((document.getElementById('autolysTime') as HTMLSelectElement | null)?.value || '60');
    const numFolds = parseInt((document.getElementById('scheduleFolds') as HTMLSelectElement | null)?.value || '3');
    const bakingMethod = ((document.querySelector('input[name="bakingMethod"]:checked') as HTMLInputElement | null)?.value || 'dutch-oven') as BakingMethod;
    ...
    const starterAmount = dataSource === 'calculated'
        ? parseFloat((document.getElementById('starter') as HTMLInputElement | null)?.value || '100')
    ...
    const saltAmount = dataSource === 'calculated'
        ? parseFloat((document.getElementById('salt') as HTMLInputElement | null)?.value || '10')
 ...
 */
function generateBulkTimerSection(
    bulkStartTime: Date,
    bulkEndTime: Date,
    bulkDurationHours: number,
...
function displaySchedule(
    ...
    bulkStartTime: Date | null = null,
    bulkEndTime: Date | null = null,
 ...
 */
function isStepChecked(stepId: string): boolean {
    try {
        const checkedSteps = JSON.parse(localStorage.getItem('scheduleCheckedSteps') || '[]') as string[];
 ...
 */
window.toggleStepCheck = function(stepId: string): void {
    try {
        const checkedSteps = JSON.parse(localStorage.getItem('scheduleCheckedSteps') || '[]') as string[];
        ...
        const checkbox = document.getElementById(stepId) as HTMLInputElement | null;
 ...
 */
function clearCheckedSteps(): void {
    try {
 ...
 */
function showContextualHint(): void {
 ...
 */
function updateTimebasedHint(startTime: Date, endTime: Date): void {
 ...
 */
window.startBulkTimerFromSchedule = function(durationHours: number): void {
 ...
 */
window.removeBulkTimer = function(): void {
 ...
 */
window.updateTimerButtons = function(): void {
    const startBtn = document.getElementById('scheduleTimerStartBtn') as HTMLButtonElement | null;
    const pauseBtn = document.getElementById('scheduleTimerPauseBtn') as HTMLButtonElement | null;
 ...
 */
function updateScheduleBreadEmojis(): void {
    const numLoaves = parseFloat((document.getElementById('scheduleNumLoavesMain') as HTMLInputElement | null)?.value || '2');
 ...
 */
function updateTotalDoughWeight(): void {
    const dataSource = (document.querySelector('input[name="scheduleDataSource"]:checked') as HTMLInputElement | null)?.value;
    ...
    if (dataSource === 'calculated') {
        ...
        const flour = parseFloat((document.getElementById('flour') as HTMLInputElement | null)?.value || '0');
        const water = parseFloat((document.getElementById('water') as HTMLInputElement | null)?.value || '0');
        const starter = parseFloat((document.getElementById('starter') as HTMLInputElement | null)?.value || '0');
        const salt = parseFloat((document.getElementById('salt') as HTMLInputElement | null)?.value || '0');
        const totalWeight = flour + water + starter + salt;
        ...
        const numLoaves = parseFloat((document.getElementById('manualNumLoaves') as HTMLInputElement | null)?.value || '1');
        const weightPerLoaf = parseFloat((document.getElementById('manualLoafWeight') as HTMLInputElement | null)?.value || '800');
